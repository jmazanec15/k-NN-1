...
public class KNN990DerivedSourceStoredFieldsReader extends StoredFieldsReader {
     ...
     */
    public KNN990DerivedSourceStoredFieldsReader(
    ...
    private KNN990DerivedSourceStoredFieldsReader(
        StoredFieldsReader delegate,
        List<DerivedFieldInfo> derivedVectorFields,
        DerivedSourceReaders derivedSourceReaders,
        SegmentReadState segmentReadState,
    ...
    private boolean doesVisitorNeedVectors(StoredFieldVisitor delegate) {
        if (delegate instanceof FieldsVisitor) {
            return derivedSourceVectorTransformer.shouldInject(Strings.EMPTY_ARRAY, Strings.EMPTY_ARRAY);
        }
        return true;
    }
    ...
    public StoredFieldsReader clone() {
        try {
            return new KNN990DerivedSourceStoredFieldsReader(
                delegate.clone(),
                derivedVectorFields,
    ...
    public static StoredFieldsReader wrapForMerge(StoredFieldsReader storedFieldsReader) {
        if (storedFieldsReader instanceof KNN990DerivedSourceStoredFieldsReader) {
            return ((KNN990DerivedSourceStoredFieldsReader) storedFieldsReader).cloneForMerge();
        }
